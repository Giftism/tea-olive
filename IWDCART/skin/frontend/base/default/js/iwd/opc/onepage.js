...
OnePage.prototype = {
    ...
    decorateSelect: function (select) {
        ...
        if (select.find('option').length && select.is(':visible')) {
            var newSelectClass = '';
            if (select.attr('data-can-unselect') && (!select.find('option:selected').html().trim().length && !select.find('option:selected').attr('data-image')) || select.attr('multiple')) {
            ...
            select.find('option').each(function () {
                ...
                if (option.html().trim().length || option.attr('data-image')) {
